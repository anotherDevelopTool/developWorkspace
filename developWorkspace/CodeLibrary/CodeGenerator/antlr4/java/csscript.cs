...
public class Script
{
    ...
    public static void Main(string[] args)
    {
        var setting = new {
            ...
            start_rule="file",
            grammar= "JavaLexer",
            g4= @".\compiled\JavaLexer.g4"
        };
        var parserSetting = new
        {
            java = "java.exe",
            target = AppDomain.CurrentDomain.BaseDirectory + @"antlr4\" + "antlr4-csharp-4.6.6-complete.jar",
            runtime = AppDomain.CurrentDomain.BaseDirectory + @"antlr4\" + "Antlr4.Runtime.dll",
            ...
            start_rule = "file",
            grammar = "JavaParser",
            g4 = @".\compiled\JavaParser.g4"
        };
        string input = args[0];

        System.IO.File.WriteAllText(@"{g4}".FormatWith(setting), args[1]);

        string genetorCommand = @"{java} -cp ""{target}"" org.antlr.v4.CSharpTool -o . -encoding UTF-8 -Dlanguage=CSharp -package {name_space}  {g4}".FormatWith(setting);
        DevelopWorkspace.Base.Logger.WriteLine(genetorCommand);
        executeJavaCommand(genetorCommand);

        System.IO.File.WriteAllText(@"{g4}".FormatWith(setting), args[1]);

        string genetorCommand = @"{java} -cp ""{target}"" org.antlr.v4.CSharpTool -o . -encoding UTF-8 -Dlanguage=CSharp -package {name_space}  {g4}".FormatWith(parserSetting);
        DevelopWorkspace.Base.Logger.WriteLine(genetorCommand);
        executeJavaCommand(genetorCommand);
        ...
        System.Reflection.Assembly addinAssembly = System.Reflection.Assembly.LoadFrom(@"{dest}{grammar}.dll".FormatWith(parserSetting));
        Type lexerType = addinAssembly.GetType("{name_space}.{grammar}".FormatWith(setting));
        Type parserType = addinAssembly.GetType("{name_space}.{grammar}".FormatWith(parserSetting));
